...
const lancerDistrib = () => {
    let carburantSelectionne = props.listeCarbu.find((carburant) => carburant.nom === selectedCarburant.value)
    if (limite.value === 0) return

    if (isLitres.value && carburantSelectionne.contenu >= limite.value) carburantSelectionne.contenu -= limite.value

    else if (isEuros.value && carburantSelectionne.contenu >= limiteEnEuros.value) carburantSelectionne.contenu -= limiteEnEuros.value
